...
function create_sample_temperature_boundaries(; nlat = 64, nlon = 128, time_dependent = false)
    ...
    if time_dependent
        ...
        create_timedep_temperature_inner_bc(
            "cmb_temp_timedep.nc", theta, phi, time, nlat, nlon, ntime)

        # Outer boundary (surface) - time-dependent cooling
        create_timedep_temperature_outer_bc(
            "surface_temp_timedep.nc", theta, phi, time, nlat, nlon, ntime)
    else
...
function main()
    ...
    println()
    println("# Create a matching transform grid")
    println("cfg = create_shtns_config(lmax=31, nlat=64, nlon=128, nr=32)")
    println()
    println("# Load temperature boundaries")
    println("temp_bc = load_temperature_boundaries_from_files(\"cmb_temp.nc\", \"surface_temp.nc\", cfg)")
    println("print_boundary_info(temp_bc.boundary_set)")
    println()
    println("# Load composition boundaries")
    println("comp_bc = load_composition_boundaries_from_files(\"cmb_composition.nc\", \"surface_composition.nc\", cfg)")
    println("print_boundary_info(comp_bc.boundary_set)")
    println("```")
end

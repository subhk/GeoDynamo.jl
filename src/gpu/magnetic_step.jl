...
function gpu_magnetic_field_step!(tor, pol, u_r, u_θ, u_φ, config, nlops,
    ...
    nlt_r = gpu_scratch!(ws, :mstep_nltr, tor.spec_r); nlt_i = gpu_scratch!(ws, :mstep_nlti, tor.spec_i)
    nlp_r = gpu_scratch!(ws, :mstep_nlpr, pol.spec_r); nlp_i = gpu_scratch!(ws, :mstep_nlpi, pol.spec_i)
    ...
    z = gpu_scratch!(ws, :mstep_z, tor.spec_r, (nl, nm)); fill!(z, zero(eltype(tor.spec_r)))
    bcin_tor_r = gpu_scratch!(ws, :mstep_bitr, z); bcin_tor_i = gpu_scratch!(ws, :mstep_biti, z)

...
function update_boundary_conditions_for_timestep!(state, current_time::Float64)
    # Refresh each field independently so optional subsystems can be absent
    # without forcing a separate timestep path for every reduced-physics case.
...
function apply_boundary_conditions_to_rhs!(rhs, state, field_type::FieldType)
    ...
    # Modern BC handling is embedded in the linear solve. This hook stays in
    # place so timesteppers can follow one boundary protocol even when the real
    # enforcement happens inside matrix assembly and solve routines.
...
function get_boundary_data(field, field_type::FieldType)
    # Returning `nothing` here is intentional: these field families no longer
    # expose a standalone boundary object because the BC rows live in the
    # implicit operator rather than a post-processing layer.
...
function get_field_from_state(state, field_type::FieldType)
    # Runtime structs differ slightly across drivers, so boundary code goes
    # through this narrow normalization point instead of hard-coding layout.
    if field_type == TEMPERATURE
...
function apply_temperature_bc_to_rhs!(rhs, temp_field)
    ...
    if hasfield(typeof(temp_field), :boundary_values)
        ...
        for lm in 1:nlm
            ...
            bc_type_inner = hasfield(typeof(temp_field), :bc_type_inner) ?
                            temp_field.bc_type_inner[lm] : Int(DIRICHLET)
            bc_type_outer = hasfield(typeof(temp_field), :bc_type_outer) ?
                            temp_field.bc_type_outer[lm] : Int(DIRICHLET)
...
function apply_composition_bc_to_rhs!(rhs, comp_field)
    ...
    if hasfield(typeof(comp_field), :boundary_values)
        ...
        for lm in 1:nlm
            bc_type_inner = hasfield(typeof(comp_field), :bc_type_inner) ?
                            comp_field.bc_type_inner[lm] : Int(DIRICHLET)
            bc_type_outer = hasfield(typeof(comp_field), :bc_type_outer) ?
                            comp_field.bc_type_outer[lm] : Int(DIRICHLET)
...
function enforce_boundary_conditions_in_solution!(solution, state, field_type::FieldType)
    ...
    # Some schemes still need an explicit post-solve correction even when newer
    # paths embed BCs algebraically, so the dispatcher remains centralized here.
...
function compute_boundary_condition_residual(field, field_type::FieldType)
    # Residual evaluation is diagnostic-only: it compares the current field
    # against prescribed boundaries without mutating the active solution state.
    # Get boundary data using unified interface
    ...
    for (field_type, field_name) in zip(field_types, field_names)
        ...
        if field !== nothing
            ...
            if boundary_set !== nothing
                ...
                if boundary_set.inner_boundary.is_time_dependent ||

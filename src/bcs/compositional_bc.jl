...
#
# Included by: src/compositional.jl
#
# ================================================================================
...
#
#   3. Fill boundary rows based on i_cmp_bc:
#      - Dirichlet: identity row (diagonal = 1)
#      - Neumann: copy first derivative operator row from D%dr(1)
#      - Special: if i_cmp_bc=4 and l=0, override inner to Dirichlet
#
...
function create_composition_matrices(config::SHTnsKitConfig,
                                      ...
                                      diffusivity::Float64,
                                      dt::Float64;
                                      i_cmp_bc::Int=get_parameters().i_cmp_bc,
    ...
    for (idx, l) in enumerate(unique_l)
        ...
        # Apply boundary conditions based on i_cmp_bc
        # (matching Fortran cmp_bc_C)

        # --- Inner boundary ---
        if i_cmp_bc == 1 || i_cmp_bc == 2
        ...
        # Special case: if both boundaries are Neumann (i_cmp_bc=4) and l=0,
...
1. RHS boundary rows are set to prescribed values (cmp_setbc)

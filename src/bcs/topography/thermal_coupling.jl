...
function apply_thermal_topography_correction!(
    ...
    if ∂r !== nothing && domain !== nothing
        # Thermal coupling needs boundary derivatives of the perturbation field;
        # compute them once here so both ICB and CMB corrections can reuse the
        # same radial traces.
        cache = compute_boundary_derivative_cache(spectral, ∂r, ∂²r, domain)
    else
...
function compute_dirichlet_thermal_correction(l::Int, m::Int,
    ...
    # Dirichlet coupling has two pieces: the field shift term h·∂rΘ and the
    # conductive-profile correction that adjusts the imposed boundary value.

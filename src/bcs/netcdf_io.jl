...
function read_netcdf_boundary_data(filename::String; precision::Type{T} = Float64) where {T}
    ...
    NCDataset(filename, "r") do ds
        ...
        field_vars = [
            "temperature", "velocity", "magnetic", "composition", "u", "v", "w", "b", "xi"]
        field_name = nothing
        field_data = nothing
        ...
        description = haskey(ds[field_name].attrib, "long_name") ?
                      ds[field_name].attrib["long_name"] : field_name
    ...
    NCDataset(filename, "r") do ds
        ...
        if haskey(ds, "theta")
            ...
            if minimum(theta) < 0.0 || maximum(theta) > π
                push!(errors,
                    "Theta coordinates out of range [0, π]: [$(minimum(theta)), $(maximum(theta))]")
            end
        end
...
function get_netcdf_file_info(filename::String)
    ...
    NCDataset(filename, "r") do ds
        ...
        # Global attributes
        info["global_attributes"] = Dict(name => ds.attrib[name]

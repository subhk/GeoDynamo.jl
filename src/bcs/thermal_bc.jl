...
#
#   3. Fill boundary rows based on i_tmp_bc:
#      - Dirichlet: identity row (diagonal = 1)
#      - Neumann: copy first derivative operator row from D%dr(1)
#      - Special: if i_tmp_bc=4 and l=0, override inner to Dirichlet
#
...
function create_temperature_matrices(config::SHTnsKitConfig,
                                      ...
                                      diffusivity::Float64,
                                      dt::Float64;
                                      i_tmp_bc::Int=get_parameters().i_tmp_bc,
    ...
    for (idx, l) in enumerate(unique_l)
        ...
        # Apply boundary conditions based on i_tmp_bc
        # (matching Fortran tmp_bc_T)

        # --- Inner boundary ---
        if i_tmp_bc == 1 || i_tmp_bc == 2
        ...
        # Special case: if both boundaries are Neumann (i_tmp_bc=4) and l=0,
...
1. RHS boundary rows are set to prescribed values (tmp_setbc)

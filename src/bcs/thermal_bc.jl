...
#
# Boundary condition types controlled by temperature_bc_code parameter:
#   1: Fixed T on both ICB and CMB (Dirichlet-Dirichlet)
#   2: Fixed T on ICB, fixed flux on CMB (Dirichlet-Neumann)
#   3: Fixed flux on ICB, fixed T on CMB (Neumann-Dirichlet)
...
#
# Dirichlet: identity row in matrix (T = prescribed value)
# Neumann: first derivative row in matrix (∂T/∂r = prescribed flux)
#
# Included by: src/physics/temperature/field.jl
#
# ================================================================================
# ALGORITHM (matching Fortran tmp_matrices / tmp_bc_T / tmp_setbc)
# ================================================================================
#
#   1. Construct LHS matrix: X = (c1/dt)I - c2*θ*L  for each harmonic degree l
#      where L = diffusivity * (∂²/∂r² + 2/r ∂/∂r - l(l+1)/r²)
#      and c1=1, c2=1/Pr (thermal Prandtl number ratio)
#
#   2. Zero boundary rows of X (first and last rows of banded matrix)
#
#   3. Fill boundary rows based on temperature_bc_code:
#      - Dirichlet: identity row (diagonal = 1)
#      - Neumann: copy first derivative operator row from D%dr(1)
#      - Special: if temperature_bc_code=4 and l=0, override inner to Dirichlet
#
#   4. LU factorize the modified matrix
#
#   5. Before solving, set RHS boundary values (tmp_setbc: usually 0)
#
...
boundary conditions embedded in the matrix rows (matching Fortran tmp_bc_T).
...
- `dt`: Timestep
- `temperature_bc_code`: BC type (1=DD, 2=DN, 3=ND, 4=NN)
...
"""
function create_temperature_matrices(config::SHTnsKitConfig,
                                      domain::RadialDomain,
                                      diffusivity::Float64,
                                      dt::Float64;
    ...
    for (idx, l) in enumerate(unique_l)
        ...
        # Apply boundary conditions based on temperature_bc_code
        # (matching Fortran tmp_bc_T)

        # --- Inner boundary ---
        if temperature_bc_code == 1 || temperature_bc_code == 2
        ...
        # --- Outer boundary ---
        if temperature_bc_code == 1 || temperature_bc_code == 3
        ...
        # Special case: if both boundaries are Neumann (temperature_bc_code=4) and l=0,
...
Set boundary values in the RHS vector for the temperature solve.
Matches Fortran tmp_setbc: sets RHS boundary rows to prescribed values.

For standard geodynamo simulations, boundary values are typically zero
(homogeneous Dirichlet: T=0, or homogeneous Neumann: ∂T/∂r=0).
Non-zero imaginary parts are used when file-based spectral BCs are loaded.
"""
function set_temperature_rhs_bc!(rhs_real::AbstractArray{T}, rhs_imag::AbstractArray{T},
...
Solve the implicit temperature system with boundary conditions embedded in the matrix.
...
function solve_temperature_implicit_step!(solution::SHTnsSpecField{T},
    ...
    @inbounds for lm_idx in lm_range
        ...
        # Set RHS boundary values (matching Fortran tmp_setbc)

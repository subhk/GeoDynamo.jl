...
function shtnskit_vector_synthesis!(tor_spec::SHTnsSpecField{T},
    ...
    #         and poloidal (:spec_transform_pol_real/imag).
    ...
    # Step 2: transpose solve→transform for all four spectral buffers.
    transpose_solve_to_transform!(tr_tor_real, tor_spec.data_real)
    transpose_solve_to_transform!(tr_tor_imag, tor_spec.data_imag)
    transpose_solve_to_transform!(tr_pol_real, pol_spec.data_real)
    transpose_solve_to_transform!(tr_pol_imag, pol_spec.data_imag)
    ...
    r_range_ph = PencilArrays.range_local(vec_phys.θ_component.pencil)[3]
    ...
    @inbounds for (r_loc_tr, r_glob) in enumerate(r_range_tr)
        ...
        # Build partial dense coefficient matrices from this rank's l-subset.
        ...
        # Radial component v_r = l(l+1)/r² · P (MIE: /r², consistent with this path).
        if domain !== nothing
            if r_glob >= 1 && r_glob <= domain.N
...
function shtnskit_vector_analysis!(vec_phys::SHTnsVectorField{T},
    ...
    # Bridge: for each local r level, dist_analysis_sphtor the θ-local slab → dense
    # (slm, tlm) replicated on θ_comm; scatter into spec_transform (r-dist, m-full);
    # then transpose transform→solve orientation back to spec.
    ...
    # Local r-ranges (spec_transform and phys.r both r-dist, aligned).
    r_range_tr = PencilArrays.range_local(config.pencils.spec_transform)[3]
    r_range_ph = PencilArrays.range_local(vec_phys.θ_component.pencil)[3]
    ...
    @inbounds for (r_loc_tr, r_glob) in enumerate(r_range_tr)
        ...
        # Load θ-local slabs and analyse → dense (slm=pol, tlm=tor) replicated on θ_comm.
    ...
    transpose_transform_to_solve!(tor_spec.data_real, tr_tor_real)
    transpose_transform_to_solve!(tor_spec.data_imag, tr_tor_imag)
    transpose_transform_to_solve!(pol_spec.data_real, tr_pol_real)
    transpose_transform_to_solve!(pol_spec.data_imag, tr_pol_imag)
end
...
@inline function coefficient_matrix_is_local(config)
    spec_pencil = config.pencils.spec
    local_ranges = spec_pencil.axes_local
    global_sizes = spec_pencil.size_global
    return length(local_ranges[1]) == global_sizes[1] &&
           length(local_ranges[2]) == global_sizes[2]
end

@inline function build_local_spectral_lm_map(config)
    spec_pencil = config.pencils.spec
...
@inline function local_spectral_lm_map(config)
    ...
    if map === nothing
        ...
        buffers.local_spectral_lm_map = map
    end
    return map::Matrix{Int}
end
...
@inline function local_spectral_slot_lookup(config)
    ...
    if lookup === nothing
        ...
        buffers.local_spectral_slot_lookup = lookup
    end
    return lookup::Vector{CartesianIndex{2}}
end
...
@inline function local_spectral_mode_indices(config)
    ...
    if modes === nothing
        ...
        buffers.local_spectral_mode_indices = modes
    end
    return modes::Vector{Int}
end

@inline function local_spectral_storage_slot(config, lm_idx::Int)
    slot = local_spectral_slot_lookup(config)[lm_idx]
    return slot == CartesianIndex(0, 0) ? nothing : slot
end
...
@inline function gather_local_radial_profile!(profile_real, profile_imag,
    ...
    for r_idx in r_range
        ...
        if local_r <= size(spec_real, 3) && local_r <= size(spec_imag, 3)
            profile_real[r_idx] = spec_real[slot[1], slot[2], local_r]
            profile_imag[r_idx] = spec_imag[slot[1], slot[2], local_r]
        end
    end
    return profile_real, profile_imag
end
...
@inline function scatter_local_radial_profile!(spec_real, spec_imag,
    ...
    for r_idx in r_range
        local_r = r_idx - first(r_range) + 1
        if local_r <= size(spec_real, 3) && local_r <= size(spec_imag, 3)
            spec_real[slot[1], slot[2], local_r] = profile_real[r_idx]
            spec_imag[slot[1], slot[2], local_r] = profile_imag[r_idx]
        end
    end
...
function store_coefficients_from_shtnskit_mapped!(spec_real, spec_imag, coeffs_matrix,
    ...
    for slot in CartesianIndices(lm_map)
        ...
        if lm_idx == 0
            spec_real[slot[1], slot[2], r_local] = 0.0
            spec_imag[slot[1], slot[2], r_local] = 0.0
            continue
        end
...
function extract_coefficients_for_shtnskit(spec_real, spec_imag, r_local, config)
    ...
    # When the spectral pencil already contains the full coefficient matrix, the
    # local extraction result is complete and no collective is needed.
    coefficient_matrix_is_local(config) && return coeffs_buffer

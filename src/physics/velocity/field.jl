...
"""
function create_velocity_workspace(::Type{T}, nr::Int, nthreads::Int=max(Threads.nthreads(), Threads.maxthreadid())) where T
...
function compute_vorticity_spectral_full!(𝒰::SHTnsVelocityFields{T},
    ...
    # This wrapper owns workspace policy: keep the hot vorticity kernel free of
    # allocation concerns while still giving callers a simple one-function API.
    # =========================================================================
...
function _compute_vorticity_spectral_threaded!(𝒰::SHTnsVelocityFields{T}, domain::RadialDomain,
    ...
    nT = max(1, Threads.nthreads(), Threads.maxthreadid())
    ...
    # Parallelism is over spectral modes because each mode owns an independent
    # radial ODE-like stencil once the `(ℓ,m)` profile has been extracted.

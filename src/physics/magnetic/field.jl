...
import .bcs
import .bcs: BoundaryType, DIRICHLET, NEUMANN, NEUMANN_MAG_INNER, NEUMANN_MAG_OUTER,
             CONTINUITY_MAG
...
function _spectral_curl_torpol!(
    ...
    for lm_idx in lm_range
        ...
        gather_local_radial_profile!(
            Pᴾ_profile_real, Pᴾ_profile_imag, src_pol_r, src_pol_i, slot, r_range)
...
function compute_current_density_spectral!(ℬ::SHTnsMagneticFields{T},
    ...
    _spectral_curl_torpol!(
        ...
        ℬ.l_factors, ℬ.∂r, ℬ.∂²r, outer_core_domain, ℬ.toroidal.config, T;
        _work = ℬ.curl_work
    )
end
...
function compute_curl_of_induction!(ℬ::SHTnsMagneticFields{T}) where {T}
    ...
    _spectral_curl_torpol!(
        ...
        parent(ℬ.work_tor.data_real), parent(ℬ.work_tor.data_imag),
        parent(ℬ.work_pol.data_real), parent(ℬ.work_pol.data_imag),
        ℬ.l_factors, ℬ.∂r, ℬ.∂²r, ℬ.outer_domain, ℬ.toroidal.config, T;
        _work = ℬ.curl_work
    )
end
...
"""
function validate_magnetic_configuration(ℬ::SHTnsMagneticFields{T}, config::C) where {
        T, C <: SHTnsKitConfig}
    errors = String[]
    ...
    if local_mode_count > local_slot_capacity
        push!(errors,
            "Toroidal magnetic field local slot capacity is smaller than owned spectral mode count")
    end
    ...
    if local_mode_count > size(ℬ.toroidal_ic.data_real, 1) * size(ℬ.toroidal_ic.data_real, 2)
        push!(errors,
            "Inner core toroidal field local slot capacity is smaller than owned spectral mode count")
    end

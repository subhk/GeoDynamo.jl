...
function Base.show(io::IO, ::MIME"text/plain", sim::Simulation)
    ...
    println(io, "├── callbacks: ", isempty(sim.callbacks) ? "(none)" :
                                   join(keys(sim.callbacks), ", "))
    print(io, "└── output_writers: ",
        isempty(sim.output_writers) ? "(none)" : join(keys(sim.output_writers), ", "))
end

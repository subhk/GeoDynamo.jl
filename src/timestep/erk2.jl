...
@eval GeoDynamo begin
    ...
    function get_erk2_cache! end
    function get_erk2_magnetic_toroidal_cache! end
    function get_erk2_magnetic_poloidal_cache! end
    function get_erk2_temperature_cache! end
    function get_erk2_composition_cache! end
...
"""
    compat_seed_solver_erk2_cache!(solver_caches, solver_key, caches, public_key)

Copy a public compatibility cache into the solver-local cache dictionary.
"""
function compat_seed_solver_erk2_cache!(
    solver_caches::Dict{Symbol, ERK2StageCache{T}},
    solver_key::Symbol,
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    public_key::Symbol,
) where {T}
    haskey(caches, public_key) || return solver_caches
    solver_caches[solver_key] = compat_solver_erk2_cache(caches[public_key])
    return solver_caches
end

@inline compat_solver_erk2_boundary_side(side::SolverERK2BoundarySide{T}) where {T} = side

@inline compat_solver_erk2_boundary_spec(::Nothing) = nothing
@inline compat_solver_erk2_boundary_spec(spec::SolverERK2BoundarySpec{T}) where {T} = spec
...
"""
    GeoDynamo.get_erk2_cache!(caches, key, diffusivity, T, config, domain, dt; ...)

Public dictionary-backed get-or-build wrapper for generic ERK2 caches.
"""
function GeoDynamo.get_erk2_cache!(
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    key::Symbol,
    diffusivity::Float64,
    ::Type{T},
    ...
    m::Int=20,
    tol::Float64=1e-8,
    bc_spec::Union{Nothing, GeoDynamo.ERK2BoundarySpec{T}, SolverERK2BoundarySpec{T}}=nothing,
) where {T}
    solver_caches = Dict{Symbol, ERK2StageCache{T}}()
    compat_seed_solver_erk2_cache!(solver_caches, key, caches, key)
    solver_cache = get_solver_erk2_cache!(
        solver_caches,
        key,
        diffusivity,
        T,
        ...
        m,
        tol,
        bc_spec=compat_solver_erk2_boundary_spec(bc_spec),
    )
    cache = compat_old_erk2_cache(solver_cache)
    caches[key] = cache
    return cache
end

"""
    GeoDynamo.get_erk2_magnetic_toroidal_cache!(caches, diffusivity, T, config, domain, dt; ...)

Public get-or-build wrapper for magnetic toroidal ERK2 caches.
"""
function GeoDynamo.get_erk2_magnetic_toroidal_cache!(
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    diffusivity::Float64,
    ::Type{T},
    config::SHTnsConfigType,
    ...
    public_key = :magnetic_toroidal_embedded
    solver_key = :solver_magnetic_toroidal_embedded
    solver_caches = Dict{Symbol, ERK2StageCache{T}}()
    compat_seed_solver_erk2_cache!(solver_caches, solver_key, caches, public_key)
    solver_cache = get_solver_erk2_magnetic_toroidal_cache!(
        solver_caches,
        diffusivity,
        T,
        config,
    ...
    cache = compat_old_erk2_cache(solver_cache)
    caches[public_key] = cache
    return cache
end

"""
    GeoDynamo.get_erk2_magnetic_poloidal_cache!(caches, diffusivity, T, config, domain, dt; ...)

Public get-or-build wrapper for magnetic poloidal ERK2 caches.
"""
function GeoDynamo.get_erk2_magnetic_poloidal_cache!(
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    diffusivity::Float64,
    ::Type{T},
    config::SHTnsConfigType,
    ...
    public_key = :magnetic_poloidal_embedded
    solver_key = :solver_magnetic_poloidal_embedded
    solver_caches = Dict{Symbol, ERK2StageCache{T}}()
    compat_seed_solver_erk2_cache!(solver_caches, solver_key, caches, public_key)
    solver_cache = get_solver_erk2_magnetic_poloidal_cache!(
        solver_caches,
        diffusivity,
        T,
        config,
...
"""
    GeoDynamo.get_erk2_temperature_cache!(caches, diffusivity, T, config, domain, dt, temperature_bcs; ...)

Public get-or-build wrapper for temperature ERK2 caches.
"""
function GeoDynamo.get_erk2_temperature_cache!(
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    diffusivity::Float64,
    ::Type{T},
    config::SHTnsConfigType,
    ...
    temperature_bc_code = _thermal_bc_code(temperature_bcs)
    public_key = Symbol(:temperature_embedded_bc, temperature_bc_code)
    solver_key = Symbol(:solver_temperature_embedded_bc, temperature_bc_code)
    solver_caches = Dict{Symbol, ERK2StageCache{T}}()
    compat_seed_solver_erk2_cache!(solver_caches, solver_key, caches, public_key)
    solver_cache = get_solver_erk2_temperature_cache!(
        solver_caches,
        diffusivity,
        T,
        config,
...
"""
    GeoDynamo.get_erk2_composition_cache!(caches, diffusivity, T, config, domain, dt, composition_bcs; ...)

Public get-or-build wrapper for composition ERK2 caches.
"""
function GeoDynamo.get_erk2_composition_cache!(
    caches::Dict{Symbol, GeoDynamo.ERK2Cache{T}},
    diffusivity::Float64,
    ::Type{T},
    config::SHTnsConfigType,
    ...
    composition_bc_code = _composition_bc_code(composition_bcs)
    public_key = Symbol(:composition_embedded_bc, composition_bc_code)
    solver_key = Symbol(:solver_composition_embedded_bc, composition_bc_code)
    solver_caches = Dict{Symbol, ERK2StageCache{T}}()
    compat_seed_solver_erk2_cache!(solver_caches, solver_key, caches, public_key)
    solver_cache = get_solver_erk2_composition_cache!(
        solver_caches,
        diffusivity,
        T,
        config,
...
"""
    _get_solver_erk2_cache!(caches, key, label, diffusivity, T, config, domain, dt; ...)

Dictionary-backed build-or-reuse wrapper for generic ERK2 stage caches.
"""
function _get_solver_erk2_cache!(
    caches::Dict{Symbol, ERK2StageCache{T}},
    key::Symbol,
    label::AbstractString,
    diffusivity::Float64,
    ::Type{T},
    ...
    caches[key] = _get_or_build_erk2_cache(
    ...
    return caches[key]::ERK2StageCache{T}
end

"""
    _get_solver_erk2_scalar_cache!(caches, key, label, diffusivity, T, config, domain, dt, boundary_condition; ...)

Dictionary-backed build-or-reuse wrapper for scalar ERK2 stage caches.
"""
function _get_solver_erk2_scalar_cache!(
    caches::Dict{Symbol, ERK2StageCache{T}},
    key::Symbol,
    label::AbstractString,
    diffusivity::Float64,
    ...
    caches[key] = _get_or_build_erk2_scalar_cache(
        get(caches, key, nothing),
        label,
        diffusivity,
        T,
...
"""
    get_solver_erk2_temperature_cache!(...)

Return a temperature ERK2 cache, rebuilding it if timestep, grid, diffusivity,
boundary code, or Krylov settings changed.
"""
function get_solver_erk2_temperature_cache!(
    ...
    key = Symbol(:solver_temperature_embedded_bc, temperature_bc_code)
    return _get_solver_erk2_scalar_cache!(
        caches,
        key,
        "temperature",
        diffusivity,
...
"""
    get_solver_erk2_composition_cache!(...)

Return a composition ERK2 cache with the same rebuild rules as temperature.
"""
function get_solver_erk2_composition_cache!(
    ...
    key = Symbol(:solver_composition_embedded_bc, composition_bc_code)
    return _get_solver_erk2_scalar_cache!(
        caches,
        key,
        "composition",
...
"""
    get_solver_erk2_cache!(...)

Return a generic velocity ERK2 cache from either dictionary-backed or
`TimestepCaches` storage.
"""
...
"""
    get_solver_erk2_magnetic_toroidal_cache!(...)

Return the magnetic toroidal ERK2 cache, rebuilding when parameters change.
"""
function get_solver_erk2_magnetic_toroidal_cache!(
    ...
    key = :solver_magnetic_toroidal_embedded
    nr = domain.N
    ...
    if needs_rebuild
        ...
        caches[key] = create_solver_erk2_magnetic_toroidal_cache(
            T,
            config,
            domain,
            diffusivity,
...
"""
    get_solver_erk2_magnetic_poloidal_cache!(...)

Return the magnetic poloidal ERK2 cache, rebuilding when parameters change.
"""
function get_solver_erk2_magnetic_poloidal_cache!(
    ...
    key = :solver_magnetic_poloidal_embedded
    nr = domain.N
    existing = get(caches, key, nothing)
    needs_rebuild =
        existing === nothing ||
    ...
    if needs_rebuild
        ...
        caches[key] = create_solver_erk2_magnetic_poloidal_cache(
            T,
            config,
            domain,
            diffusivity,
...
Return cached velocity-poloidal influence operators for the current grid and
timestep parameters.
"""
function get_solver_erk2_influence_matrices!(
    cache::Dict{Symbol, ERK2InfluenceCacheEntry{T}},
    key::Symbol,
    ::Type{T},
    config::SHTnsConfigType,
    domain::RadialDomainType,
    ...
    cache[key] = _get_or_build_erk2_influence_entry(

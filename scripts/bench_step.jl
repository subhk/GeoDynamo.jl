...
function main()
    ...
    # Warm-up
    GeoDynamo.compute_vorticity_spectral_full!(state.velocity, state.𝒟ᵒᶜ)
    GeoDynamo.compute_all_nonlinear_terms!(state.velocity, state.temperature, nothing, state.magnetic, state.𝒟ᵒᶜ)

...
@testset "User API" begin
    ...
    @testset "BoundaryConditions codes" begin
        ...
        @test GeoDynamo._velocity_bc_code(GeoDynamo.BoundaryConditions(inner = ns, outer = ns)) ==
              1
        @test GeoDynamo._velocity_bc_code(GeoDynamo.BoundaryConditions(inner = ns, outer = sf)) ==
              2
        @test GeoDynamo._velocity_bc_code(GeoDynamo.BoundaryConditions(inner = sf, outer = ns)) ==
              3
        @test GeoDynamo._velocity_bc_code(GeoDynamo.BoundaryConditions(inner = sf, outer = sf)) ==
              4
        @test GeoDynamo._thermal_bc_code(GeoDynamo.BoundaryConditions(inner = ff, outer = ft)) ==
              3
    end

...
function __allocation_static_source(path_parts...)
    return read(joinpath(ALLOCATION_STATIC_SRC, path_parts...), String)
end

function __allocation_static_function_body(source::String, signature::String)
...
@testset "Hot-path allocation source checks" begin
    numerics = __allocation_static_source("solver", "numerics.jl")
    vorticity_body = __allocation_static_function_body(
        numerics,
    ...
    backend = __allocation_static_source("solver", "backend.jl")
    @test occursin("theta_full_real::Vector{T}", backend)
    @test occursin("theta_full_imag::Vector{T}", backend)

    nonlinear = __allocation_static_source("physics", "nonlinear.jl")
    theta_body = __allocation_static_function_body(
        nonlinear,
    ...
    scalar_transform_body = __allocation_static_function_body(
        nonlinear,
    ...
    state = __allocation_static_source("solver", "state.jl")
    imex = __allocation_static_source("timestep", "imex.jl")
    velocity_solver = __allocation_static_source("physics", "velocity", "solver.jl")
    ...
    simulation = __allocation_static_source("api", "simulation.jl")
    ...
    file_bc_loader = __allocation_static_source("bcs", "file_bc_loader.jl")
    @test occursin("mutable struct BoundaryInterpolationCache{T", file_bc_loader)

    solver_numerics = __allocation_static_source("solver", "numerics.jl")
    @test !occursin("cache[\"bc_real\"]::Matrix", solver_numerics)
    @test !occursin("cache[\"bc_imag\"]::Matrix", solver_numerics)

    temperature_field = __allocation_static_source("physics", "temperature", "field.jl")
    composition_field = __allocation_static_source("physics", "composition", "field.jl")
    ...
    spectral = __allocation_static_source("transforms", "spectral.jl")
    ...
    @test !occursin("field = get(__BUFFERS_FIELD_MAP", spectral)
    @test occursin("function get_cached_buffer!(create_func::F, config, ::Val{key}) where {F,key}", spectral)

    containers = __allocation_static_source("fields", "containers.jl")
    ...
    for source in (
        __allocation_static_source("physics", "temperature", "field.jl"),
        __allocation_static_source("physics", "composition", "field.jl"),
        __allocation_static_source("physics", "velocity", "field.jl"),
        __allocation_static_source("physics", "magnetic", "field.jl"),
    )
        @test !occursin("config::SHTnsKitConfig", source)
    end

    backend_state = __allocation_static_source("GeoDynamo.jl")
    ...
    solver_backend = __allocation_static_source("solver", "backend.jl")
    ...
    imex = __allocation_static_source("timestep", "imex.jl")
    krylov_body = __allocation_static_function_body(
        imex,
    ...
    @test occursin("krylov_work=radial_work", __allocation_static_source("physics", "temperature", "solver.jl"))
    @test occursin("krylov_work=radial_work", __allocation_static_source("physics", "composition", "solver.jl"))
    @test occursin("krylov_work=radial_work", __allocation_static_source("physics", "magnetic", "solver.jl"))
    ...
    magnetic_field = __allocation_static_source("physics", "magnetic", "field.jl")
    @test occursin("curl_work::NTuple{6,Vector{T}}", magnetic_field)
    current_body = __allocation_static_function_body(
        magnetic_field,
        "function compute_current_density_spectral!(",
    )
    induction_body = __allocation_static_function_body(
        magnetic_field,
        "function compute_curl_of_induction!(",
    )
    @test occursin("__work=ℬ.curl_work", current_body)
    @test occursin("__work=ℬ.curl_work", induction_body)

    solver_current_body = __allocation_static_function_body(
        solver_numerics,
        "function solver_compute_current_density_spectral!(",
    )
    solver_induction_body = __allocation_static_function_body(
        solver_numerics,
        "function solver_compute_curl_of_induction!(",
    )
    @test occursin("__work=magnetic_fields.curl_work", solver_current_body)
    @test occursin("__work=magnetic_fields.curl_work", solver_induction_body)

    @test !occursin("solver_get_cached_buffer!(create_func::Function", nonlinear)
    @test !occursin("field = get(__SOLVER_BUFFERS_KEY_MAP", nonlinear)
    @test occursin("function solver_get_cached_buffer!(create_func::F, config, ::Val{key}) where {F,key}", nonlinear)

    scalar_ops = __allocation_static_source("fields", "scalar_operators.jl")
    flux_tau_body = __allocation_static_function_body(
        scalar_ops,
        "function apply_flux_bc_tau!(",
    )
    influence_body = __allocation_static_function_body(
        scalar_ops,
    ...
    solver_state = __allocation_static_source("solver", "state.jl")
    erk2 = __allocation_static_source("timestep", "erk2.jl")
    integrate_erk2_body = __allocation_static_function_body(
        erk2,
        "function integrate_solver_erk2_step!(",
    )
    finalize_erk2_body = __allocation_static_function_body(
        erk2,
    ...
    temperature_solver = __allocation_static_source("physics", "temperature", "solver.jl")
    composition_solver = __allocation_static_source("physics", "composition", "solver.jl")
    magnetic_solver = __allocation_static_source("physics", "magnetic", "solver.jl")
    ...
    velocity_field = __allocation_static_source("physics", "velocity", "field.jl")
    @test !occursin("__compute_vorticity_spectral_threaded!", velocity_field)
end

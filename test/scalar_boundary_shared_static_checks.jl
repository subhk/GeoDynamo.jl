...
function __scalar_bc_shared_source(path_parts...)
    return read(joinpath(SCALAR_BC_SHARED_ROOT, path_parts...), String)
end

function __scalar_bc_shared_function_body(source::String, signature::String)
...
@testset "Shared scalar boundary-condition implementation" begin
    geodynamo = __scalar_bc_shared_source("src", "GeoDynamo.jl")
    ...
    thermal_bc = __scalar_bc_shared_source("src", "bcs", "thermal_bc.jl")
    composition_bc = __scalar_bc_shared_source("src", "bcs", "compositional_bc.jl")
    ...
    temperature_matrices = __scalar_bc_shared_function_body(
        thermal_bc,
        "function create_temperature_matrices(",
    )
    composition_matrices = __scalar_bc_shared_function_body(
        composition_bc,
    ...
    temperature_solve = __scalar_bc_shared_function_body(
        thermal_bc,
        "function solve_temperature_implicit_step!(",
    )
    composition_solve = __scalar_bc_shared_function_body(
        composition_bc,

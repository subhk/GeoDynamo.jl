...
@testset "Spectral operators (dense alm)" begin
    ...
    @testset "inverse Laplacian round-trips l≥1, zeros l=0" begin
        ...
        lap = newalm(); G.apply_horizontal_laplacian!(cfg, a0; alm_out=lap)
        inv = newalm(); G.apply_inverse_horizontal_laplacian!(cfg, lap; alm_out=inv)
    ...
    @testset "compute_horizontal_gradient_magnitude = Σ l(l+1)|a|² (×2 for m>0)" begin
        ...
        b = newalm(); set!(b, 2, 0, 3.0 + 0im)                         # 6·|3|²·1
        @test G.compute_horizontal_gradient_magnitude(cfg, b) ≈ 6 * 9
        c = newalm(); set!(c, 2, 1, 3.0 + 0im)                         # 6·|3|²·2 (m>0)
    ...
    @testset "apply_spectral_filter! applies filter_func(l,m)" begin
        a = newalm()
        set!(a, 1, 0, 2.0 + 0im); set!(a, 2, 0, 4.0 + 0im); set!(a, 2, 1, 1.0 + 1im)
        ref = copy(a)
        G.apply_spectral_filter!(cfg, a, (l, m) -> 0.5)               # halve all
        @test a ≈ 0.5 .* ref
        b = newalm(); set!(b, 1, 0, 2.0 + 0im); set!(b, 2, 0, 4.0 + 0im)
    ...
    @testset "rotate_field_z!: α=0 / α=2π identity, m=0 fixed, energy invariant" begin
        a = newalm()
        set!(a, 1, 0, 1.0 + 0im); set!(a, 2, 1, 0.7 - 0.3im); set!(a, 3, 2, -0.4 + 0.9im)
        e0 = sum(abs2, a)

        id0 = newalm(); G.rotate_field_z!(cfg, a, 0.0; alm_out=id0)
        @test id0 ≈ a                                                 # zero rotation
        id2 = newalm(); G.rotate_field_z!(cfg, a, 2π; alm_out=id2)
        @test id2 ≈ a                                                 # full turn
        rot = newalm(); G.rotate_field_z!(cfg, a, 0.37; alm_out=rot)
    ...
    @testset "rotations leave an l=0 monopole invariant (native or fallback)" begin
        mono = newalm(); set!(mono, 0, 0, 2.5 + 0im)

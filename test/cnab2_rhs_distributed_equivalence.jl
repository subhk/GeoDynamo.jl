...
@testset "CNAB2 + ERK2 distributed equivalence (radial-local ⇒ comm-free)" begin
    ...
    function fill_det!(field, salt)
        ...
        for lm in GeoDynamo.local_spectral_mode_indices(cfg)
            ...
            for r in 1:nr
                GeoDynamo.set_local_spectral_value!(fr, slot, r, sinpi(0.05 * (lm + salt) +
                                                                       0.013 * r))
                GeoDynamo.set_local_spectral_value!(fi, slot, r, cospi(0.04 * (lm - salt) +
                                                                       0.017 * r))
            end
        end
        return field
    ...
    ur = parent(u.data_real);
    ui = parent(u.data_imag)
    nr_ = parent(nl.data_real);
    ni_ = parent(nl.data_imag)
    pr = parent(pv.data_real);
    pi_ = parent(pv.data_imag)
    ...
    lgr = parent(rhs_legacy.data_real);
    lgi = parent(rhs_legacy.data_imag)
    legacy_maxerr = 0.0
    ...
    for lm in GeoDynamo.local_spectral_mode_indices(cfg)
        ...
        for r in 1:nr
            ref = inv_dt * GeoDynamo.local_spectral_value(ur, slot, r) +
                  ...
                  0.5 * GeoDynamo.local_spectral_value(pr, slot, r) +
                  linear_weight * Lu[r]
            legacy_maxerr = max(legacy_maxerr, abs(GeoDynamo.local_spectral_value(lgr, slot, r) -
                                                   ref))
        end
        for r in 1:nr
            uvec[r] = GeoDynamo.local_spectral_value(ui, slot, r)
        ...
        for r in 1:nr
            ref = inv_dt * GeoDynamo.local_spectral_value(ui, slot, r) +
                  ...
                  0.5 * GeoDynamo.local_spectral_value(pi_, slot, r) +
                  linear_weight * Lu[r]
            legacy_maxerr = max(legacy_maxerr, abs(GeoDynamo.local_spectral_value(lgi, slot, r) -
                                                   ref))
        end
    end
    ...
    lin_r = erk2_buf.linear_real;
    lin_i = erk2_buf.linear_imag
    k1r = erk2_buf.k1_real;
    k1i = erk2_buf.k1_imag
    str = erk2_buf.stage_real;
    sti = erk2_buf.stage_imag
    half_dt = dt / 2
    erk2_maxerr = 0.0
    uv = zeros(Float64, nr);
    nv = zeros(Float64, nr)
    ...
    for lm in GeoDynamo.local_spectral_mode_indices(cfg)
        ...
        ci == 0 && continue
        Ef = erk2_cache.E_full[ci];
        Eh = erk2_cache.E_half[ci]
        p1f = erk2_cache.phi1_full[ci];
        p1h = erk2_cache.phi1_half[ci]
        ...
        for (src, lin_b, k1_b, st_b) in ((ur, lin_r, k1r, str), (ui, lin_i, k1i, sti))
            ...
            sref[nr] = 0.0
            for r in 1:nr
                erk2_maxerr = max(erk2_maxerr, abs(GeoDynamo.local_spectral_value(lin_b, slot, r) -
                                                   lref[r]))
                erk2_maxerr = max(erk2_maxerr, abs(GeoDynamo.local_spectral_value(k1_b, slot, r) -
                                                   kref[r]))
                erk2_maxerr = max(erk2_maxerr, abs(GeoDynamo.local_spectral_value(st_b, slot, r) -
                                                   sref[r]))
            end
        end
    end
    ...
    fr_u = parent(u.data_real);
    fi_u = parent(u.data_imag)
    fin_maxerr = 0.0
    ...
    for lm in GeoDynamo.local_spectral_mode_indices(cfg)
        ...
        for (lin_b,
            ...
            snl_b,
            out_b) in (
            (erk2_buf.linear_real, erk2_buf.k1_real,
                erk2_buf.n_current_real, erk2_buf.stage_nl_real, fr_u),
            (erk2_buf.linear_imag, erk2_buf.k1_imag,
                erk2_buf.n_current_imag, erk2_buf.stage_nl_imag, fi_u))
            for r in 1:nr
            ...
            for r in 1:nr
                ref = GeoDynamo.local_spectral_value(lin_b, slot, r) +
                      ...
                      2 * dt * corr[r]
                (r == 1 || r == nr) && (ref = 0.0)
                fin_maxerr = max(fin_maxerr, abs(GeoDynamo.local_spectral_value(out_b, slot, r) -
                                                 ref))
            end
        end
    end

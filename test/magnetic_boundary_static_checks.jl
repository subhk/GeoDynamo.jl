...
function __magnetic_bc_static_source(path_parts...)
    return read(joinpath(MAGNETIC_BC_STATIC_ROOT, path_parts...), String)
end

function __magnetic_bc_static_function_body(source::String, signature::String)
...
@testset "Magnetic boundary-condition static contract" begin
    magnetic_bc = __magnetic_bc_static_source("src", "bcs", "magnetic_bc.jl")
    magnetic_solver = __magnetic_bc_static_source("src", "physics", "magnetic", "solver.jl")
    erk2 = __magnetic_bc_static_source("src", "timestep", "erk2.jl")

    tor_matrices = __magnetic_bc_static_function_body(
        magnetic_bc,
        "function create_magnetic_toroidal_matrices(",
    )
    pol_matrices = __magnetic_bc_static_function_body(
        magnetic_bc,
    ...
    legacy_magnetic_solve = __magnetic_bc_static_function_body(
        magnetic_bc,
        "function solve_magnetic_implicit_step!(",
    )
    toroidal_inner_bc = __magnetic_bc_static_function_body(
        magnetic_solver,
    ...
    insulating_inner = __magnetic_bc_static_function_body(
        erk2,
        "function solver_create_insulating_inner_bc(",
    )
    insulating_outer = __magnetic_bc_static_function_body(
        erk2,
    ...
    magnetic_tor_update = __magnetic_bc_static_function_body(
        magnetic_solver,
        "function apply_magnetic_toroidal_implicit_update!(",
    )
    magnetic_pol_update = __magnetic_bc_static_function_body(
        magnetic_solver,
    ...
    @test occursin("__timestepper_krylov_dimension(timestepper, state.parameters)", magnetic_tor_update)
    @test occursin("__timestepper_krylov_dimension(timestepper, state.parameters)", magnetic_pol_update)
    @test !occursin("__timestepper_krylov_dimension(state.parameters.timestepper)", magnetic_tor_update)
    @test !occursin("__timestepper_krylov_dimension(state.parameters.timestepper)", magnetic_pol_update)

    integrate_erk2 = __magnetic_bc_static_function_body(
        erk2,

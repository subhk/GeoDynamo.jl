...
function __temperature_bc_static_source(path_parts...)
    return read(joinpath(TEMPERATURE_BC_STATIC_ROOT, path_parts...), String)
end

function __temperature_bc_static_function_body(source::String, signature::String)
...
@testset "Temperature boundary-condition static contract" begin
    api = __temperature_bc_static_source("src", "api", "boundary_conditions.jl")
    scalar_bc = __temperature_bc_static_source("src", "bcs", "scalar_bc.jl")
    thermal_bc = __temperature_bc_static_source("src", "bcs", "thermal_bc.jl")
    temperature_solver = __temperature_bc_static_source("src", "physics", "temperature", "solver.jl")
    backend = __temperature_bc_static_source("src", "solver", "backend.jl")
    numerics = __temperature_bc_static_source("src", "solver", "numerics.jl")
    imex = __temperature_bc_static_source("src", "timestep", "imex.jl")
    erk2 = __temperature_bc_static_source("src", "timestep", "erk2.jl")

    @test occursin("__thermal_bc_code(::BoundaryConditions{<:FixedTemperature, <:FixedTemperature}) = 1", api)
    @test occursin("__thermal_bc_code(::BoundaryConditions{<:FixedTemperature, <:FixedFlux})        = 2", api)
    @test occursin("__thermal_bc_code(::BoundaryConditions{<:FixedFlux,        <:FixedTemperature}) = 3", api)
    @test occursin("__thermal_bc_code(::BoundaryConditions{<:FixedFlux,        <:FixedFlux})        = 4", api)

    temperature_matrices = __temperature_bc_static_function_body(
        thermal_bc,
    ...
    scalar_rows = __temperature_bc_static_function_body(
        scalar_bc,
    ...
    legacy_temperature_solve = __temperature_bc_static_function_body(
        thermal_bc,
    ...
    state = __temperature_bc_static_source("src", "solver", "state.jl")
    boundary_spec = __temperature_bc_static_function_body(
        state,
    ...
    prepare_erk2 = __temperature_bc_static_function_body(
        erk2,
        "function prepare_solver_erk2_field!(",
    )
    finalize_erk2 = __temperature_bc_static_function_body(
        erk2,
    ...
    temperature_update = __temperature_bc_static_function_body(
        temperature_solver,
    ...
    @test occursin("__timestepper_krylov_dimension(timestepper, state.parameters)", temperature_update)
    @test !occursin("__timestepper_krylov_dimension(state.parameters.timestepper)", temperature_update)

    integrate_erk2 = __temperature_bc_static_function_body(
        erk2,

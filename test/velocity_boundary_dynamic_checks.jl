...
# ============================================================================
# Dynamic (numerical) velocity boundary-condition checks.
#
# The companion `velocity_boundary_static_checks.jl` only string-matches the
# source to confirm the BC code *exists*. This file instead builds the real
# production operators, solves a radial profile per harmonic, and asserts the
# solved field actually *satisfies* each boundary condition:
#
...
#                            ∂²P/∂r² = 0
#
# Boundary derivatives are evaluated with the same one-sided radial stencils
# the solver embeds, so these tests verify the linear solve + influence
# correction enforce exactly the discrete conditions that were specified.
# ============================================================================

@testset "Velocity boundary-condition dynamic checks" begin
    cfg = GeoDynamo.create_shtnskit_config(lmax=4, mmax=4, nlat=12, nlon=16, nr=24)
    dom = GeoDynamo.create_shell_radial_domain(24)
    N   = dom.N
    dt  = 1.0e-3
    θ   = 0.5
    ...
    @testset "Poloidal no-slip ⇒ P = 0 (influence) and ∂P/∂r = 0" begin
        ...
        infl = GeoDynamo.create_velocity_poloidal_influence_matrices(Float64, cfg, dom, 1.0, dt, bc_ns_ns; theta = θ)
    ...
    @testset "Poloidal stress-free ⇒ P = 0 (influence) and ∂²P/∂r² = 0" begin
        ...
        infl = GeoDynamo.create_velocity_poloidal_influence_matrices(Float64, cfg, dom, 1.0, dt, bc_sf_sf; theta = θ)
        GeoDynamo.apply_influence_matrix_correction!(P, infl[l])
        d2P = d2 * P

...
!------------------------------------------------------------------------
   subroutine non_velocity()
      ...
      do n = 1, mes_oc%pN
         do m = 0, i_Ph-1
            r%Re(:j,m,n)  = r%Re(:j,m,n)  &
               +  mag_curlt%Re(:j,m,n)*mag_p%Re(:j,m,n)/d_Pm  &
               -  mag_curlp%Re(:j,m,n)*mag_t%Re(:j,m,n)d_Pm 
            th%Re(:j,m,n) = th%Re(:j,m,n)  &
               +  mag_curlp%Re(:j,m,n)*mag_r%Re(:j,m,n)/d_Pm  &
               -  mag_curlr%Re(:j,m,n)*mag_p%Re(:j,m,n)/d_Pm
            ph%Re(:j,m,n) = ph%Re(:j,m,n)  &
               +  mag_curlr%Re(:j,m,n)*mag_t%Re(:j,m,n)/d_Pm  &
               -  mag_curlt%Re(:j,m,n)*mag_r%Re(:j,m,n)/d_Pm
         end do
      end do
      ...
      call var_spec2coll(sq,cq, s2=ss,c2=cs, s3=st,c3=ct)
         			...
         			! thermal Buoyancy
      d = d_PrT * qRaT * mes_oc%r(:,1)
      do nh = 0, cq%H%pH1
         do n = 1, mes_oc%N
            cq%Re(n,nh) = cq%Re(n,nh) + d(n)*tmp_T%Re(n,nh)
            cq%Im(n,nh) = cq%Im(n,nh) + d(n)*tmp_T%Im(n,nh)
         end do
      end do
      
               			! compositional Buoyancy
      d = d_PrC * qRaC * mes_oc%r(:,1)
      do nh = 0, cq%H%pH1
         do n = 1, mes_oc%N
            cq%Re(n,nh) = cq%Re(n,nh) + d(n)*cmp_C%Re(n,nh)
            cq%Im(n,nh) = cq%Im(n,nh) + d(n)*cmp_C%Im(n,nh)
         end do
      end do
...
!------------------------------------------------------------------------
   subroutine non_temperature()
      ...
      r%Re(:j,:,:n) =  &
         - vel_r%Re(:j,:,:n) * tmp_gradr%Re(:j,:,:n)  &
         - vel_t%Re(:j,:,:n) * tmp_gradt%Re(:j,:,:n)  &
         - vel_p%Re(:j,:,:n) * tmp_gradp%Re(:j,:,:n)

      call tra_phys2spec(r, ss)
      call var_spec2coll(ss, tmp_NC)
         			! internal source
      if(mpi_rnk==0)  tmp_NC%Re(:,0) = tmp_NC%Re(:,0) + tmp_S
...
!------------------------------------------------------------------------
   ...
   subroutine non_composition()
      ...
      r%Re(:j,:,:n) =  &
         - vel_r%Re(:j,:,:n) * cmp_gradr%Re(:j,:,:n)  &
         - vel_t%Re(:j,:,:n) * cmp_gradt%Re(:j,:,:n)  &
         - vel_p%Re(:j,:,:n) * cmp_gradp%Re(:j,:,:n)

      call tra_phys2spec(r, ss)
      call var_spec2coll(ss, cmp_NC)
         			! internal source
      if(mpi_rnk==0)  cmp_NC%Re(:,0) = cmp_NC%Re(:,0) + cmp_S

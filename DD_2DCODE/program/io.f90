...
!**************************************************************************
   ...
   character(200)        :: io_statefile, io_tmpBCfile, io_cmpBCfile, io_magB0file
   integer,     private  :: io_save1, io_save2
   integer,     private  :: io_vE, io_mE, io_mIC, io_mMB
   integer,     private  :: io_tNu, io_cNu, io_rTq, io_tdt, io_pts
...
!--------------------------------------------------------------------------
   subroutine io_precompute()
      ...
      io_tmpBCfile = 'tmpBC.cdf.in'
      io_cmpBCfile = 'cmpBC.cdf.in'
      ...
      if(b_tmp_tstep .and.  &
        (i_tmp_bc==1 .or. i_tmp_bc==3)) io_tNu = 50
      
      if(b_cmp_tstep .and.  &
        (i_cmp_bc==1 .or. i_cmp_bc==3)) io_cNu = 55
...
!--------------------------------------------------------------------------
   subroutine io_openfiles()
      ...
      if(io_tNu/=0)  open(io_tNu, status='unknown', file='tmp_nusselt.dat')
      if(io_cNu/=0)  open(io_cNu, status='unknown', file='cmp_nusselt.dat')
...
!--------------------------------------------------------------------------
   subroutine io_write2files()
      ...
      if(modulo(tim_step,i_save_rate1)==0) then
         ...
         call io_save_spectrum1('tmp',tmp_C,1d0)
         call io_save_spectrum1('cmp',cmp_C,1d0)
         if(b_mag_impose)  &
      ...
      if(modulo(tim_step,i_save_rate2)==0) then
         ...
         if(io_tNu/=0) call io_write_nussel(io_tNu,tmp_C)
         if(io_cNu/=0) call io_write_nussel(io_cNu,cmp_C)
...
!   with a random noise of small amplitude)
      ...
      call io_load_coll(f, 'T',into,No_,roc,Ao, tmp_T)
      call io_load_coll(f, 'C',into,No_,roc,Ao, cmp_C)
...
!--------------------------------------------------------------------------
!  Load temperature boundary condition and source info
!--------------------------------------------------------------------------
   subroutine io_load_tmpBC()
      ...
      e=nf90_open(io_tmpBCfile, nf90_nowrite, f)      
      if(e/=nf90_noerr) then
         if(mpi_rnk==0) then 
            tmp_bcRe(1,0) = 1d0
            if (i_tmp_bc==2 .or. i_tmp_bc==4) tmp_bcRe(2,0) = 1d0
            print*, 'WARNING! tmpBC file not found, set values:'
            print*, '   tmp_S          : S = 0'
            print*, '   tmpBC l=0 inner:', real(tmp_bcRe(1,0))
            print*, '   tmpBC l=0 outer:', real(tmp_bcRe(2,0))
         end if
         return
      end if
      ...
      fn = 0d0
      e=nf90_inq_varid(f, 'T', i)
      e=nf90_get_var(f, i, fn)
      if(into) then
         call io_interp(No_,roc,fn,Ao,No,mes_oc%r(1,1), tmp_T)
      else
         tmp_S = fn
      end if
...
!            print*, '   cod_S max      :', real(maxval(cod_S))
   ...
   end subroutine io_load_tmpBC


!--------------------------------------------------------------------------
!  Load temperature boundary condition and source info
!--------------------------------------------------------------------------
   subroutine io_load_cmpBC()
      integer :: e, f, i, rd, i9,j9
      integer :: n, No, No_
      double precision :: d
      integer, parameter :: lda = 2*i_KL+1
      ...
      e=nf90_open(io_cmpBCfile, nf90_nowrite, f)      
      if(e/=nf90_noerr) then
         if(mpi_rnk==0) then 
            tmp_bcRe(1,0) = 1d0
            if (i_cmp_bc==2 .or. i_cmp_bc==4) tmp_bcRe(2,0) = 1d0
            print*, 'WARNING! cmpBC file not found, set values:'
            print*, '   cmp_S          : S = 0'
            print*, '   cmpBC l=0 inner:', real(cmp_bcRe(1,0))
            print*, '   cmpBC l=0 outer:', real(cmp_bcRe(2,0))
         end if
         return
      end if
      ...
      fn = 0d0
      e=nf90_inq_varid(f, 'C', i)
      e=nf90_get_var(f, i, fn)
      if(into) then
         call io_interp(No_,roc,fn,Ao,No,mes_oc%r(1,1), cmp_C)
      else
         cmp_S = fn
      end if
   ...
   subroutine io_save_state()
     ...
     ! print*, 'io_save_state'
      ...
      call io_create_par('state'//cnum//'.cdf.dat', f)
         ...
         e=nf90_put_att(f, nf90_global,  'RaT', d_RaT)
         e=nf90_put_att(f, nf90_global,  'RaC', d_RaC)
         e=nf90_put_att(f, nf90_global,  'PrT', d_PrT)
         e=nf90_put_att(f, nf90_global,  'PrC', d_PrC)
         ...
         call io_define_coll(f,   'T', dims,tmp_T,   T)
         call io_define_coll(f,   'C', dims,cmp_C,   C)
      ...
      call io_save_coll(f,  T,i_N,tmp_T)
      call io_save_coll(f,  C,i_N,cmp_C)
...
!--------------------------------------------------------------------------
!  Save temperature boundary condition and source info
!--------------------------------------------------------------------------
   subroutine io_save_tmpBC()
      integer :: e, f
      integer :: H
      ...
      if(_Np/=1) stop 'io_save_tmpBC: set _Np 1'
      if(mpi_rnk==0) then
         print*, ' saving tmpBC.cdf.dat'
         e=nf90_create('tmpBC.cdf.dat', nf90_clobber, f)
         ...
         e=nf90_def_var(f,   'r', nf90_double, (/rd/), r)
         e=nf90_def_var(f,   'S', nf90_double, (/rd/), T)

         call io_define_coll(f, 'Tbc',(/rirod,uPHd,ReImd/),tmp_C, Tbc)

         e=nf90_enddef(f)

         e=nf90_put_var(f,     r, mes_oc%r(1:i_N,1))
         e=nf90_put_var(f,     T, tmp_S(1:i_N))
      end if

      call var_coll_init(mes_oc,var_H, cq)
      cq%Re(:2,:) = tmp_bcRe
      cq%Im(:2,:) = tmp_bcIm
      call io_save_coll(f,Tbc,2,cq)

      if(mpi_rnk==0)  &
         e=nf90_close(f)

   end subroutine io_save_tmpBC
 

!--------------------------------------------------------------------------
!  Save temperature boundary condition and source info
!--------------------------------------------------------------------------
   subroutine io_save_cmpBC()
      integer :: e, f
      integer :: H
      integer :: rd, uPHd, ReImd, rirod
      integer :: r, C,Cbc
      ...
      if(_Np/=1) stop 'io_save_cmpBC: set _Np 1'
      if(mpi_rnk==0) then
         print*, ' saving cmpBC.cdf.dat'
         e=nf90_create('cmpBC.cdf.dat', nf90_clobber, f)
         ...
         e=nf90_def_var(f,   'r', nf90_double, (/rd/), r)
         e=nf90_def_var(f,   'C', nf90_double, (/rd/), C)

         call io_define_coll(f, 'Cbc',(/rirod,uPHd,ReImd/),cmp_C, Cbc)

         e=nf90_enddef(f)

         e=nf90_put_var(f,     r, mes_oc%r(1:i_N,1))
         e=nf90_put_var(f,     C, cmp_S(1:i_N))
      end if
...
!--------------------------------------------------------------------------
!  write the Nusselt number for temperature :  actual flux / flux for no flow  at cmb
!--------------------------------------------------------------------------
   subroutine io_write_nussel_tmp(ifile, C)
      integer,     intent(in) :: ifile
      ...
      integer :: N, j
      if(mpi_rnk/=0) return
      if(C%H%pH0>0) stop 'io_write_nussel for temperature'
      if(cflx==0d0) call tmp_cmbflux(1d0/d_PrT, cflx)
      N = C%D%N
      flx = 0d0
      do j = N-i_KL, N
   ...
   end subroutine io_write_nussel_tmp


!--------------------------------------------------------------------------
!  write the Nusselt number for composition:  actual flux / flux for no flow  at cmb
!--------------------------------------------------------------------------
   subroutine io_write_nussel_cmp(ifile, C)
      integer,     intent(in) :: ifile
      type (coll), intent(in) :: C
      double precision, save  :: cflx = 0d0
      double precision :: flx
      integer :: N, j
      if(mpi_rnk/=0) return
      if(C%H%pH0>0) stop 'io_write_nussel for composition'
      if(cflx==0d0) call cmp_cmbflux(1d0/d_PrC, cflx)
      N = C%D%N
      flx = 0d0
      do j = N-i_KL, N
         flx = flx  +  C%Re(j,0) * C%D%dr(1)%M(i_KL+1+N-j,j)
      ...
      write(ifile,'(2e20.12)') tim_t, flx/cflx
   end subroutine io_write_nussel_cmp
...
!--------------------------------------------------------------------------
   subroutine io_write_bench(ifile)
      ...
      call var_coll2spec(tmp_T, sC)
      call tra_spec2phys(sC, pC1)
      call var_coll2spec(cmp_C, sC)
      call tra_spec2phys(sC, pC2)
      ...
      if(r_==mpi_rnk) then
         ...
         dat(3) =    pC1%Re(Nth,0,Nr) + pC2%Re(Nth,0,Nr) 	! C
